...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={cn(
        "h-screen w-64 bg-slate-900 text-white flex flex-col fixed left-0 top-0 z-50 transition-transform duration-300 ease-in-out md:translate-x-0",
        ...
        <div className="p-6 border-b border-slate-800 flex justify-between items-center">
          ...
          <button onClick={onClose} className="md:hidden text-slate-400 hover:text-white">
            <LogOut className="h-5 w-5 rotate-180" /> {/* Reusing LogOut icon as close/back for now or just click outside */}
          </button>
        </div>
